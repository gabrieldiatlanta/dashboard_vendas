...
quantidade_vendas = dados.groupby('Local da compra')[['Produto']].count()
quantidade_vendas = dados.drop_duplicates(subset = 'Local da compra')[['Local da compra', 'lat', 'lon']].merge(quantidade_vendas, left_on = 'Local da compra', right_index = True).sort_values('Produto', ascending = False)

quantidade_vendas_estados = pd.DataFrame(dados.groupby('Local da compra')[['Produto']].count().sort_values('Produto', ascending=False))
...
quantidade_vendas_categoria = dados.groupby('Categoria do Produto')[['Produto']].count().sort_values('Produto', ascending=False)
...
fig_mapa_quantidade = px.scatter_geo(quantidade_vendas,
                                   lat = 'lat',
                                   lon = 'lon',
                                   scope = 'south america',
                                   size = 'Produto',
...
fig_quantidade_estados = px.bar(quantidade_vendas_estados.head(),
                                                                text_auto = True,
                                                                title = 'Top 5 - Quantidade de vendas por estado')
fig_quantidade_estados.update_layout(yaxis_title = 'Top 5 - Quantidade de vendas')

fig_quantidade_mensal = px.line(quantidade_vendas_mensal,
                                                        x = 'Mês',
                                                        y = 'Produto',
                                                        markers = True,
...
fig_quantidade_categoria = px.bar(quantidade_vendas_categoria,
                                                                text_auto = True,
                                                                title = 'Quantidade de vendas por categoria')

fig_quantidade_categoria.update_layout(yaxis_title = 'Quantidade de vendas')
